"use strict";
function getAudioTrackAsync(test) {
    ...
    waitForEventAndRunStep('sourceended', source, test.step_func_done(function(evt) {
        if (!!buffer) {
            if (!('audioTracks' in video))
                throw new InstantiationError('video.audioTracks is not defined.');
            if (!video.audioTracks)
                throw new InstantiationError('video.audioTracks is null.');
            if (video.audioTracks.length < 1)
                throw new InstantiationError('video.audioTracks.length is zero.');
            level1TestInstance(video.audioTracks[0], test.properties.def);
        }
    }), test);
