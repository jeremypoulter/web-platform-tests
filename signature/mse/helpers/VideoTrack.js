"use strict";
function getVideoTrackAsync(test) {
    ...
    waitForEventAndRunStep('sourceended', source, test.step_func_done(function(evt) {
        if (!!buffer) {
            if (!('videoTracks' in video))
                throw new InstantiationError('video.videoTracks is not defined.');
            if (!video.videoTracks)
                throw new InstantiationError('video.videoTracks is null.');
            if (video.videoTracks.length < 1)
                throw new InstantiationError('video.videoTracks.length is zero.');
            level1TestInstance(video.videoTracks[0], test.properties.def);
        }
    }), test);
